...
public class MySQLConnection {
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        //因为item实际上再twitch上，我们不可能把所有的item都insert到我们自己的Item table里，所以我们选择favorite一个，就insert一个到
        //Item这个table里。
        saveItem(item);
    ...
    //注意返回的是Map,key可以是 video, clip, stream; 对应的value是List 有点不太明白?????
        ...
        try {
            ...
            for (String itemId : favoriteItemIds) {
                ...
                if (rs.next()) {
                    ...
                    itemMap.get(rs.getString("type")).add(item);//key为3中type，所以只有三个，value各对应一个List.
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
